...
def get_action_value(mdp, state_values, state, action, gamma):
    """ Computes Q(s,a) """
    q = np.sum([p * (mdp.get_reward(state, action, next_state) +  gamma * state_values[next_state]) for next_state, p in mdp.get_next_states(state,action).items()])
    return q
...
def get_optimal_action(mdp, state_values, state, gamma=0.9):
    ...
    for action in mdp.get_possible_actions(state):
        curr_q = get_action_value(mdp, state_values, state, action, gamma)
        if curr_q > max_Q:
            max_Q = curr_q
            max_action = action
    assert not (max_action is None)
    return max_action
